...
# 团队成员：**不写学号**。PPT 与主页都会进公开仓库，学号属于身份信息，
# 需要学号的地方（提交专区表单）由成员自己填写。
TEAM = [
    {"name": "黄宇科",
     "role": "主程",
     "desc": "代码 / 部署 / 版本管理 / 自评测工具"},
    {"name": "张镒川",
     "role": "产品与材料",
     "desc": "报告脱敏 / 评分点整理 / 独立人工打分 / 试用 / PPT / 视频"},
]
TEAM_AFFILIATION = "深圳大学计算机与软件学院 · 计算机科学与技术 · 2025 级 05 班"
...
def main():
    ...
    txt(s, 0.95, 5.5, 11.4, 0.9,
        [("粤港澳大湾区 AI Coding 创新大赛 · 方向一：AI + 教学管理助手",
          {"size": 13, "color": MUTED}),
         ("团队成员：" + "、".join(m["name"] for m in TEAM)
          + "（深圳大学计算机与软件学院）　·　开源协议：MIT",
          {"size": 13, "color": MUTED})])
    ...
    txt(s, 0.9, 4.14, 11.6, 0.95,
        [("评测方法：9 份真实实验报告由非主程成员独立人工打分（gold set）——"
    ...
    card(s, 0.62, 1.72, 6.0, 2.3, "AI 帮我们抓到的两个真实缺陷",
         ["① 上下文丢失：模型只看到报告 7%~11% 内容（MAE 31.67 → 20.44）",
    ...
    card(s, 0.62, 4.18, 6.0, 1.75, "团队与分工（2 人，均为计算机科学与技术专业）",
         [f"{m['name']}（{m['role']}）：{m['desc']}" for m in TEAM]
         + [TEAM_AFFILIATION], size=11)
