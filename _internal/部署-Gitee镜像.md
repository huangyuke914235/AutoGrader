...
> ⚠ **2026-09-23 更新：Gitee Pages 已于 2024 年正式下线**（官方说明"由于不可抗原因，Pages 服务已下线"），
> 所以下面第二部分起的 Gitee 步骤**不可执行**，仅作留档。国内镜像请用 **EdgeOne Pages**：
>
> 1. 打开 EdgeOne Pages 控制台（腾讯云账号登录）→ 新建项目 → **从 GitHub 导入** `AutoGrader`
> 2. 框架预设选"无/静态站点"，**构建命令留空**，**输出目录填 `docs`**（填错会 404）
> 3. 部署完成得到 `*.edgeone.app` 域名；以后 push 会自动重新部署（比 Gitee 手动点更新省事）
>
> 也可以**不做镜像**，直接交 GitHub Pages 链接，并在材料里注明"如访问缓慢可用离线案例"。

# Gitee Pages 镜像部署（**已失效，仅留档**）
